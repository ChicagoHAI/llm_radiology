...
## adapted from https://github.com/ysmiura/ifcc

import os
from data.areport import AReportData
from data.chexpert import CheXpertData
from data.flickr30k import Flickr30kData
from data.mimiccxr import MIMICCXRData
from data.openi import OpenIData
...
class Data:
    ...
    def get_datasets(cls, path, corpus, word_indexes, sentsplitter, tokenizer, textfilter, tokenfilter, max_sent,
                     ...
                     filter_reports=True, split='train', args = None):
        # datasets = {}
        ...
        if corpus == 'a':
            ...
            if split == 'train' and not args.training.debug:
                target_transform = MIMICCXRData.get_target_transform(word_indexes, 'train', sentsplitter, tokenizer,
                                                                     textfilter, tokenfilter, max_sent, max_word)
                dataset = MIMICCXRData(args, path, section=section, split='train',
                                                 target_transform=target_transform, cache_image=cache, cache_text=cache,
                                                 multi_image=multi_image, img_mode=img_mode, img_augment=img_augment,
                                                 dump_dir=train_dump, filter_reports=filter_reports)
            elif split == 'train' and args.training.debug:
                target_transform = MIMICCXRData.get_target_transform(word_indexes, 'train', sentsplitter, tokenizer,
                                                                     textfilter, tokenfilter, max_sent, max_word)
                dataset = MIMICCXRData(args, path, section=section, split='validation',
                                                 target_transform=target_transform, cache_image=cache, cache_text=cache,
                                                 multi_image=multi_image, img_mode=img_mode, img_augment=img_augment,
                                                 dump_dir=val_dump, filter_reports=filter_reports, debug=True)
            elif split == 'validation':
                target_transform = MIMICCXRData.get_target_transform(word_indexes, 'validation', sentsplitter, tokenizer,
                                                                 textfilter, tokenfilter, max_sent, max_word)
                dataset = MIMICCXRData(args, path, section=section, split='validate',
                ...
                target_transform = MIMICCXRData.get_target_transform(word_indexes, 'test', sentsplitter, tokenizer,
                                                                 textfilter, tokenfilter, max_sent, max_word)
                dataset = MIMICCXRData(args, path, section=section, split='test', target_transform=target_transform,
