...
from torchvision.transforms import CenterCrop, ColorJitter, Compose, Normalize, RandomApply, RandomRotation, Resize, ToTensor, RandomHorizontalFlip
from torchvision.transforms.functional import pad
from data.text.sentsplit import get_sentsplitter
from data.text.textfilter import get_textfilter
from data.text.tokenfilter import get_tokenfilter
from data.text.tokenizer import get_tokenizer
...
class _CaptioningData(data.Dataset):
    ...
    def _transform(cls, cache_image, resize_dim=256, mode='center', augment=False):
        ...
        if augment:
            flip = RandomApply([RandomHorizontalFlip()], p=0.5)
            rotate = RandomApply([RandomRotation(15.0, expand=True)], p=0.5)
            # color = RandomApply([ColorJitter(brightness=0.1, contrast=0.1)], p=0.5)
            augs = [flip, rotate]
            if mode == cls.TRANSFORM_CENTER:
                augs += [Resize(resize_dim), CenterCrop(256)] ## 224
            elif mode == cls.TRANSFORM_PAD:
                augs += [PadSquare(), Resize(256)]
            else:
        ...
        if cache_image:
            if mode == cls.TRANSFORM_CENTER:
                return Compose([Resize(resize_dim), CenterCrop(256)]), Compose(augs + [ToTensor(), norm])
            elif mode == cls.TRANSFORM_PAD:
                return Compose([PadSquare(), Resize(256)]), Compose(augs + [ToTensor(), norm])
            else:
                raise ValueError('Unknown transform mode {0}'.format(mode))
        else:
            if mode == cls.TRANSFORM_CENTER:
                return None, Compose([Resize(resize_dim), CenterCrop(256)] + augs + [ToTensor(), norm])
            elif mode == cls.TRANSFORM_PAD:
                return None, Compose([PadSquare(), Resize(256)] + augs + [ToTensor(), norm])
            else:
                raise ValueError('Unknown transform mode {0}'.format(mode))
